...
scene_width = int(screen_width * 0.985 + 20)  # 80% of screen width
scene_height = int(screen_height * 0.85 + 20)  # 60% of screen height
...
scene = canvas(title='3D Solar System ',
               ...
               autoscale=False,
               title_color=color.black)  # Set title color here
...
for i, data in enumerate(planets_data):
    planet = sphere(pos=vector(data['orbital_radius'], 0, 0),
                    ...
                    make_trail=False,
                    shininess=0,
                    name=data['name'])  # Assign a name to the planet
...
# Test function for planet clicks
def planet_clicked(evt):
    print("Mouse click detected!")  # Simple test print
...
# Try binding to the scene's window
if hasattr(scene, 'window'):
    scene.window.bind('mousedown', planet_clicked)
else:
    scene.bind('mousedown', planet_clicked) # Fallback to scene binding

# Add a horizontal slider below the canvas and couple it to time_speed
def slider_callback(s):
    global time_speed
    time_speed = s.value  # Link slider value to simulation speed

# Append slider below the canvas
