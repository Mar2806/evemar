...
scene_width = int(screen_width * 0.985)  # 80% of screen width
scene_height = int(screen_height*0.9) # 60% of screen height
...
scene = canvas(title='3D Solar System ',
                ...
                autoscale=False,
                title_color=color.white)  # Set title color here

# Set the title color using HTML styling
scene.title = "<div style='color:white'>3D Solar System </div>"
...
# Position the slider
slider_x = 10
slider_y = scene_height - 150  # Position at the bottom
time_speed_slider = slider(pos=vector(slider_x, slider_y, 0),
                           min=1, max=100, value=time_speed, length=200,
                           caption='Simulation Speed', bind=dummy_function,
                           color=color.white, background=color.black)  # Set slider colors

# Set the background color of the entire page.  This requires using Javascript, so LEAVE OUT IN FINAL PART
scene.append_to_caption('<script>document.body.style.backgroundColor = "black";</script>')
