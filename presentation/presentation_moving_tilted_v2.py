...
horizontal_speed = 0.05  # Adjust for the speed of the horizontal motion of the Sun
trail_delay_time = 2  # Time in seconds before trails start
orbit_angle = math.radians(29.8)  # Angle of the orbital plane relative to the sun's motion
...
while True:
    ...
    for i, planet in enumerate(planets):
        ...
        # Calculate the new x, y, and z coordinates for the angled orbit.  Rotate the tilt by swapping x and y
        y_orbit = data['orbital_radius'] * math.cos(angles[i] + data['phi_offset']) # Now y depends on cos
        x_orbit = data['orbital_radius'] * math.sin(angles[i] + data['phi_offset']) * math.sin(orbit_angle)  # x-component of the orbit
        z_orbit = data['orbital_radius'] * math.sin(angles[i] + data['phi_offset']) * math.cos(orbit_angle)  # z-component
