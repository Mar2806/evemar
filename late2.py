...
animation_paused = False  # New flag to control animation state
...
def planet_clicked(evt):
    ...
    if clicked_object:
        if hasattr(clicked_object, 'name'):
            ...
            animation_paused = True  # Pause animation when planet is clicked
        else:
...
def slider_callback(s):
    global time_speed, animation_paused
    time_speed = s.value
    animation_paused = False  # Resume animation when slider is adjusted
